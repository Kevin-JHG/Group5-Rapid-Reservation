...
import { IconCalendar, IconClock, IconUsers, IconShoppingCart, IconEdit, IconPlus, IconMinus } from '@tabler/icons-react';
...
const Summary = ({ setModalOpened, modalOpened, setActive }) => {
  ...
  return (
    ...
    <Text c="dimmed" mt="sm" ta="center" className={classes.subtitle}>
      ...
      <div>
        <Text className={classes.detailItem}><IconCalendar size={16} /> Date: {date ? date.toDateString() : 'Not specified'}</Text>
        <Text className={classes.detailItem}><IconClock size={16} /> Time: {time || 'Not specified'}</Text>
        <Text className={classes.detailItem}><IconUsers size={16} /> Party size: {partySize || 'Not specified'}</Text>
      </div>
            ...
            .map((item) => (
              <div key={item.id} className={classes.cartItem}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <Text size="sm" style={{ color: 'gray' }}> {quantities[item.id] + " x "+item.price.toFixed(2)+" = " +"$"+item.price.toFixed(2) *quantities[item.id]}</Text> 
                ...
                />
                <Text style={{ color: theme.colors.green[7], fontWeight: 'bold' }}>${item.price.toFixed(2)*quantities[item.id]}</Text>
              </div>
            ))}
          ...
          <Stack spacing="md">
            ...
            {loading ? (
              ...
              <List spacing="md" size="md" center icon={
                ...
                {menuDetails.filter(item => quantities[item.id] > 0).map(item => (
                  <>    <List.Item key={item.id} style={{ backgroundColor: '#dbeafe', borderRadius: '8px', padding: '10px 16px' }}>
                    
                     <Group position="apart" style={{ width: '100%', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center',justifyContent: 'space-between',gap: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                      
      <img src={`./menu/${item.id}.png`} height={60} alt="Item" style={{ borderRadius: '8px' }} /> </div>
                      <div >
                        <Text fw={500} style={{ color: '#343a40' }}>{item.name}</Text>
                        <Text size="sm" c="dimmed">{item.description}</Text>
                        
                         
                       
                        <Text size="sm" style={{ color: '#343a40' }}> {quantities[item.id] + " x "+item.price.toFixed(2)+" = " +"$"+item.price.toFixed(2) *quantities[item.id]}</Text> 
                        </div>
                        <div  >
                        <NumberInput
                            variant="unstyled"
                ...
                <List.Item style={{ backgroundColor: '#dbeafe', borderRadius: '8px', padding: '10px', marginTop: '20px' }}>
                <Stack spacing="sm" style={{ width: '100%' }}>
    <Group position="apart" style={{ width: '100%' ,justifyContent: 'space-between'}}>
      <Text fw={500} style={{ color: '#343a40' }}>Subtotal:</Text>
      <Text fw={500} style={{ color: '#343a40' }}>${calculateTotalPrice()}</Text>
    </Group>
    <Group position="apart">
      <Text fw={500} style={{ color: '#343a40' }}>Estimated Tax (17%):</Text>
      <Text fw={500} style={{ color: '#343a40' }}>${(parseFloat(calculateTotalPrice()) * 0.17).toFixed(2)}</Text>
    </Group>
    <Divider my="sm" />
    <Group position="apart">
      <Text fw={700} style={{ color: '#343a40', fontSize: '1.1em' }}>Total:</Text>
      <Text fw={700} style={{ color: '#343a40', fontSize: '1.1em' }}>
        ${(parseFloat(calculateTotalPrice()) * 1.17).toFixed(2)}
              ...
              <Text c="dimmed" style={{ color: '#343a40' }}>No items in the cart.</Text>
            )}
          </Stack>
